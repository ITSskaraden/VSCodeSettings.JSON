# КАК УСТАНОВИТЬ ОЧЕНЬ БЫСТРО СРЕДУ РАЗРАБОТКИЙ VISUAL STUDIO CODE НА WINDOWS 10 / 11 ДЛЯ PYTHON
...
4. Настройка виртуальной среды: Рекомендуется создать виртуальную среду для каждого проекта, чтобы изолировать зависимости. В терминале VS Code используйте команду python -m venv имя_окружения для создания виртуальной среды.

    - pip install pipenv (установка виртуалной среды на хост-машину, только один раз)

5. Создание для проекта в папке собственную виртуальную среду.
   ...
   -  запуск виртуальной среды и вход в терминал через VSCode (pipevn shell)
         -  внутри среды VSCode терминал:
             -  pip list
             -  pip install пакет
